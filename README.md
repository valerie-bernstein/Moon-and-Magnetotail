...
- <img src="https://latex.codecogs.com/gif.latex?\text { where } [\lambda_{\odot}] \text { is a rotation by the angle } \lambda_{\odot} \text { (the Sun's ecliptic longitude) about the z-axis, and } [\epsilon,X] \text{ is a rotation by the angle } \epsilon \text{ (the obliquity of the ecliptic) about the x-axis. } " /> These two rotations represent 
